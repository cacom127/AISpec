...
- The system shall enforce lint/format rules defined in `<config file>`
  on CHANGED FILES ONLY — KHÔNG bật toàn repo. Codebase cũ sẽ sinh hàng
  nghìn lỗi, và một luật bị vi phạm khắp nơi thì sẽ bị bỏ qua hoàn toàn.
...
- **Spec-on-touch**: when a ticket touches code chưa có spec, its
  `delta-spec.md` shall ghi nhận luôn hành vi HIỆN CÓ của phần bị chạm,
  dạng `(MỚI — ghi nhận hành vi có sẵn)`, trước khi ghi thay đổi lên trên.
  Chỉ ghi nhận trong BÁN KÍNH ẢNH HƯỞNG, không phải cả module.
- Every `specs/<module>.md` shall bắt đầu bằng ô **Độ phủ** nói rõ file đó
  là MỘT PHẦN hay ĐẦY ĐỦ. Hành vi không xuất hiện trong `specs/` KHÔNG có
...
- **Quy tắc ownership endpoint**: mỗi API endpoint shall có ĐÚNG 1 module
  ...
  file đó chỉ giữ 1 dòng/endpoint (method, path, module sở hữu). Module
...
- The AI agent shall NOT invent business requirements not present in
  ...
  liệu cũ.
- The AI agent shall NOT xoá hoặc đơn giản hoá một hành vi chỉ vì nó không
